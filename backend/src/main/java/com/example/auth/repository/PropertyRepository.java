...
public interface PropertyRepository extends JpaRepository<Property, Long> {
    ...
    @Query("SELECT DISTINCT p FROM Property p " +
        ...
        "WHERE p.status = 1 " +
        "AND a.auctionStatus = 'OPEN' " +
        "AND m.status = 1 " +
        "AND (:destination IS NULL OR LOWER(p.propertyName) LIKE LOWER(CONCAT('%', :destination, '%')) " +
        ...
        "AND (:checkIn IS NULL OR a.stayStartDate = :checkIn) " +
        "AND (:checkOut IS NULL OR a.stayEndDate = :checkOut)")
    Page<Property> searchForTraveller(@Param("destination") String destination,
